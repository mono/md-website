...
You can download MonoDevelop 0.12 source from [here](/download/). The Mono download site contains packages for everything for many popular distros. You can also check the mono redcarpet channel.
...
Code completion now is more stable and supports the C# 2.0 syntax (including generics), thanks to the new parser based on [NRefactory](http://wiki.sharpdevelop.net/default.aspx/SharpDevelop.NRefactory). This parser is already integrated in the C# language binding (Matej Urbas).
